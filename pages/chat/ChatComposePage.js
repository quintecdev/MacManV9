...
import { SafeAreaView,View,Text,TouchableOpacity,TextInput,FlatList, Image,
   PermissionsAndroid
} from 'react-native';
...
import { chatdetailpageBg, svgMicroPhone, svgXmlIcWPSend } from '../../common/SvgIcons';
...
export default ({ route,navigation }) => {
  const dispatch = useDispatch();
   ...
   const{chatReciever,isInitialChat,
...
useEffect(() => {
   const unsubscribe = messaging().onMessage(async remoteMessage => {
      console.clear()
      console.log('A new FCM message arrived!', {remoteMessage,loggedUser},chatList);
      let newChat = JSON.parse(remoteMessage.data.chat)
      console.log("ChatComposePage","onMessage: ",{newChat,ToId:newChat.ToId,TechnicianID:loggedUser?.TechnicianID})
     if(remoteMessage.data.type=='TYPE_CHAT' && loggedUser?.TechnicianID == newChat.ToId && newChat.FromId == chatReciever.ToId){
         getChatList()
         ...
         RNGRP.getRealPathFromURI(imageURI.assets[0].uri).then((path) => {
            ...
            PhotoEditor.Edit({
               ...
               onDone: (imagePath) => {
                  ...
                  setEdittedImg(Platform.OS === 'android' ? `file://${imagePath}` : imagePath)
...
//    // }
 ...
 function getChatList(){
   ...
   dispatch(actionSetLoading(true))
   requestWithEndUrl(`${API_SUPERVISOR}GetChatHistory`,{RoomId:chatReciever.ChatRoomId})
   .then(res => {
 ...
 function updateChatListStatus(chatIds){
    ...
    requestWithEndUrl(`${API_SUPERVISOR}UpdateStatus`,{ChatId:chatIds,Status:3},'POST',{'content-type': 'text/json'})
    .then(res => {
    ...
    return (
        <SafeAreaView
         style={{ flex: 1,padding:8,backgroundColor:CmmsColors.chatsContainerBg}}>
            ...
            >               
                <FlatList
                data ={chatList}
                renderItem={({item,index})=><View style={{borderRadius:10,
                  paddingVertical:2,
                  paddingHorizontal:7,
                  flexDirection:'row',
                  // minWidth:100,
                  backgroundColor:item.FromId == loggedUser?.TechnicianID?CmmsColors.chatBg:'white',
                  marginHorizontal:4, 
                marginBottom:4,alignSelf:item.FromId == loggedUser?.TechnicianID?'flex-end':'flex-start'}}>
                  ...
                  //   end:4,
                    ...
                    marginStart:8,
                    color:colorWithAlpha('black',0.5)}}>11:14 am</CText>
                    {/* <FontAwesomeIcon icon="fal fa-check" /> */}
                     {item.FromId == loggedUser?.TechnicianID&&<Icon style={{marginStart:4}}name={getIconName(item.Status)} size={12} color={getIconColor(item.Status)} />}
                     </View>
             ...
             <View
             style={{flexDirection:'row',height:chat_ip_container_height,
            //  position:'absolute',bottom:0,end:0,start:0
             ...
             >
                 ...
                 style={{flex:1,
                    ...
                    borderRadius:chat_ip_container_height,backgroundColor:CmmsColors.white}}
                 >
                    <TouchableOpacity
                 style={{
                    justifyContent:'center',
                 ...
                 >
                    <Icon name="picture-o" size={24} color={CmmsColors.chatIconGreyColor} />
                    </TouchableOpacity>
                     <TextInput
                        ref={msgIpRef}
                        value = {txtMsg}
                 ...
                 >
                    <Icon name="camera" size={20} color={CmmsColors.chatIconGreyColor} />
                 </TouchableOpacity>
                 </View>
                 <TouchableOpacity
                 style={{height:chat_ip_container_height,width:chat_ip_container_height,borderRadius:chat_ip_container_height/2,
                    justifyContent:'center',
                    alignItems:'center',
                    marginStart:4,
                    // alignContent:'center',
                    ...
                    onPress={async()=>{
                     console.log("CHat Send",txtMsg,{ChatRoomId:chatReciever.ChatRoomId})
                     if(txtMsg===""){
                        //voice meesage
                        ...
                        const tempChatlist = [...chatList,{Message:txtMsg,Status:0,TempChatId:tempMsgId,FromId:loggedUser?.TechnicianID,ToId:chatReciever.ToId}]
                        setchatList(tempChatlist)
                        isInitialChat ? sendInitialChat(tempChatlist,tempMsgId):sendChat(tempChatlist,tempMsgId)
                        settxtMsg('')
                 ...
                 >
                    <SvgXml 
                  xml={
                     txtMsg===""?svgMicroPhone:svgXmlIcWPSend
                  } width={24} height={22} color='white' />
...
{/* <Text>{txtMsg}</Text> */}
      ...
      />}
         ...
         function sendInitialChat(tempChatlist=[],tempMsgId){
            ...
            bodyFormData.append("Chat",JSON.stringify({"FromId":loggedUser?.TechnicianID,"ToId":chatReciever.ToId,"Message":txtMsg,"TempChatId":tempMsgId}))
           
               if(EdittedImg){
                  bodyFormData.append("",{uri: EdittedImg, //this.state.uri[0].uri, == image path file:///storage/emulated/0/Pictures/1511787860629.jpg
                  type: 'image/jpg',
                  name: 'chat_img.jpg'})
               }
           setEdittedImg(null)
                                 ...
                                 requestWithEndUrl(`${API_SUPERVISOR}InitialChat`,bodyFormData,'POST',{'Content-Type': 'multipart/form-data'})
                                 .then(res => {
                                    if (res.status != 200) {
                                    throw Error(res.statusText);
                                    }
                                 ...
                                 .then(data => {
                                    console.log("InitialChat: ",{data})
                                       ...
                                       let chatToUpdate = tempChatlist.find(chat=>chat.TempChatId === data.TempChatId)
                                    if(data.IsSucess){
                                       
                                       console.log({chatToUpdate})
                                       chatToUpdate['Status'] = 1
                                       setchatList([...tempChatlist])
                        navigation.setParams({chatReciever:{...chatReciever,LastMsg:txtMsg,LastMsgDate:tempMsgId,LastMsgStatus:1,ChatRoomId:data.ChatRoomId}})
                        ...
                        //upDateSelectedChatRoom({...chatReciever,LastMsg:txtMsg,LastMsgDate:tempMsgId,LastMsgStatus:1,ChatRoomId:data.ChatRoomId})
                                 ...
                                 .catch(err => {
                                    ...
                                    let chatToUpdate = tempChatlist.find(chat=>chat.TempChatId === tempMsgId)
                                    chatToUpdate['Status'] = 4
                                    setchatList(tempChatlist)
                                    navigation.setParams({chatReciever:{...chatReciever,ChatRoomId:0}})
         ...
         function sendChat(tempChatlist=[],tempMsgId){
            ...
            var bodyFormData = new FormData();
            bodyFormData.append("Chat",JSON.stringify({"ChatRoomId":chatReciever.ChatRoomId,
            "FromId":loggedUser?.TechnicianID,"ToId":chatReciever.ToId,
            "Message":txtMsg,"TempChatId":tempMsgId}))
                                 ...
                                 requestWithEndUrl(`${API_SUPERVISOR}Chat`,bodyFormData,'POST',{'Content-Type':'multipart/form-data'})
                                 .then(res => {
                                    if (res.status != 200) {
                                    throw Error(res.statusText);
                                    }
                                 ...
                                 .then(data => {
                                    ...
                                    if(data.IsSucess){
                                       ...
                                       chatToUpdate['Status'] = 1
                                       setchatList([...tempChatlist])
                        navigation.setParams({chatReciever:{...chatReciever,LastMsg:txtMsg,LastMsgDate:tempMsgId,LastMsgStatus:1}})
