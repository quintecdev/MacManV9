...
export function statusDetails(status) {
	switch (status) {
		case 0:
			return { status: 'Pending', color: CmmsColors.joPending };
		case 1:
...
const JobOrderView = (props) => {
	...
	console.log('home','JobOrderView', { props });
	return (
		<View
			style={{
				backgroundColor: statusDetails(item.Status).color,
				borderRadius: 10,
				minHeight: 70,
				// marginStart: 25,
				// paddingStart: 20,
		...
		>
			{props.isSupervisor && (
				...
				>
					...
					>
						<Icon name="clock-o" size={12} color="white" /> {Math.floor(item.EHrs / 60)}h
					</CmmsText>
					<CmmsText
						style={{
							fontSize: 12,
			...
			>
				{/* {item.TNO}/ */}
				{`${item.Code}/${item.Asset}/${item.MaintenanceJobType}/${item.Location}`.replace(/^\/+|\/+$/g, '')}
			</Text>
