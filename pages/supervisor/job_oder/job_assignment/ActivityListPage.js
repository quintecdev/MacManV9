...
import {SafeAreaView,View,
    Text,ImageBackground,Dimensions, Platform,
    Image, TouchableOpacity, FlatList, ScrollView} from 'react-native'
...
export default ActivityListPage = ({navigation,route:{params}})=>{
  ...
  useEffect(()=>{
    ...
    {TypeOfActivityID:selectedActivity.TypeOfActivityID,
      FromDate:selectedActivity.FromDate,ToDate:selectedActivity.ToDate,})
    .then(res => {
    ...
    .then(data => {
      const tempTechList = data.filter(techie=>techie.ID!=selectedActivity.SEID)
  ...
  const getWorkDetailsForAssignment = () =>{
    ...
    requestWithEndUrl(`${API_SUPERVISOR}GetWorkDetailsForAssignment`,{EmpID:TechnicianID,
  ...
  const onChangeTime = ({nativeEvent}, selectedDate) => {
    ...
    console.log("onChangeTime",{selectedDate,timestamp:timestamp,nativeEvent,selectedActivity})
    const currentDate = timestamp || (isFromTime?selectedActivity?.FromDate:selectedActivity?.ToDate);
    ...
    isFromTime ? setSelectedActivity({...selectedActivity,FromDate:currentDate}):setSelectedActivity({...selectedActivity,ToDate:currentDate})
  };
            ...
            <SafeAreaView style={{ flex: 1,padding:8 }}>
              ...
              /> */}
                ...
                showsVerticalScrollIndicator={false}
                    ...
                    style={{flexDirection:'row',backgroundColor:'white',padding:4,
                    marginVertical:4,elevation:5,justifyContent:'center',alignItems:'center'}}
                    onPress={()=>{
                      if(!isLoading){
                    ...
                    >
                      ...
                      />
                      <View
                        style={{paddingHorizontal:8,paddingVertical:4,flex:1}}
  ...
  {/* {item.Code}/{item.Asset}/{item.MaintenanceJobType}/{item.Location} */}
                            ...
                            {`${item.Code}/${item.Asset}/${item.ESC}`.replace(/^\/+|\/+$/g, '')}
                        </CmmsText>
                        <CmmsText
                        style={{color:'grey'}}
                        >
                        ...
                        ><Icon name="clock-o" size={12} color="grey" /> {`JO ${item.JONo} - ${timeMinToTimeFormat(item.Ehrs)}`}
                        </CmmsText>
                        </CmmsText>
                        <CTextThin
                        ...
                        >
                            <Icon name="clock-o" size={12} color="green" /> {format(item.FromDate, 'dd/MM/yyyy hh:mm aa')} to {format(item.ToDate, 'dd/MM/yyyy hh:mm aa')}
                        </CTextThin>
                        </View>
                    </TouchableOpacity>}
                />
                ...
                >
                    ...
                    <CmmsText
                       ...
                       <TouchableOpacity
                        style={{alignSelf:'center',justifyContent:'center',alignItems:'center',marginVertical:4}}
                      ...
                      />  
                      <CmmsText style={{fontWeight:'bold'}}>{selectedActivity?.SE}</CmmsText>

                      </TouchableOpacity>  

                     {isVisibleTechnicians&& <View style={{borderWidth:1,padding:4,borderRadius:5}}>
                        <CmmsText style={{position:'absolute',top:4,start:4,fontWeight:'bold',fontSize:15}}>{AppTextData.txt_Change_Technician}</CmmsText>
                      ...
                      renderItem={({item,index})=>
                        ...
                        padding:4,
                        maxWidth:100,
                        marginVertical:4,marginHorizontal:4}}
                        onPress={()=>{
                          dispatch(actionSetLoading(true))
                          //http://213.136.84.57:4545/api/ApkSupervisor/ChangeSEForAssignment?FromDate=1635425985000&SEID=6&JOID=0&EHrs=60
                          requestWithEndUrl(`${API_SUPERVISOR}ChangeSEForAssignment`,
                          {FromDate:item.NextASTime,
                            SEID:item.ID,JOID:selectedActivity.JOID,
                            EHrs:selectedActivity.Ehrs},'POST')
                          .then(res => {
                          ...
                          .then(data => {
                            ...
                            if(data){
                              setSelectedActivity({...selectedActivity,
                                FromDate:data.FromDate,ToDate:data.ToDate,SEID:item.ID,SE:item.Name,ImagePath:item.ImagePath})
                            }
                      ...
                      numberOfLines={1}
                      >({item.MH} - <CTextThin style={{color:CmmsColors.logoBottomGreen,fontSize:10}}>{format(item.NextASTime, 'hh:mm aa')}</CTextThin>)</CTextThin>
                      ...
                      />
                        ...
                        >{AppTextData.txt_From}: </CmmsText>
                        <TouchableOpacity
                        style={{flex:4,borderWidth:1,borderColor:'grey', padding:4,flexDirection:'row',alignItems:'center',justifyContent:'center',}}
                        onPress={()=>{
                          setIsFromTime(true)
                          setMode('date')
                        ...
                        <CmmsText
                        style={{padding:4}}
                        >{format(selectedActivity?selectedActivity.FromDate:0, 'dd/MM/yyyy')}</CmmsText> 
                        </TouchableOpacity>
                        <CmmsText
                        style={{flex:1,fontWeight:'bold',marginStart:16}}
                        >{AppTextData.txt_To}: </CmmsText>
                        <TouchableOpacity
                        style={{flex:4,borderWidth:1,borderColor:'grey',padding:4,flexDirection:'row',alignItems:'center',justifyContent:'center'}}
                        onPress={()=>{
                          setIsFromTime(false)
                        ...
                        <CmmsText
                        style={{padding:4}}
                        >{format(selectedActivity?selectedActivity.ToDate:0, 'dd/MM/yyyy')}</CmmsText> 
                        </TouchableOpacity>
                        </View>
                        ...
                        <CmmsText
                        style={{padding:4}}
                        >{format(selectedActivity?selectedActivity.FromDate:0, 'hh:mm aa')}</CmmsText> 
                        </TouchableOpacity>
                        <CmmsText
                        style={{flex:1,fontWeight:'bold',marginStart:16}}
                        >{AppTextData.txt_To}: </CmmsText>
                        ...
                        <CmmsText
                        style={{padding:4}}
                        >{format(selectedActivity?selectedActivity.ToDate:0, 'hh:mm aa')}</CmmsText> 
                        </TouchableOpacity>
                        </View>
                        
                <TouchableOpacity
                style={{backgroundColor:'green',height:40,borderRadius:20,justifyContent:'center',alignItems:'center',marginVertical:16}}
                onPress={()=>{
                 ...
                 requestWithEndUrl(`${API_SUPERVISOR}UpdateWorkAssignment`,selectedActivity,"POST")
                 .then(res => {
                ...
                >
                  <CmmsText style={{color:'white'}}>{AppTextData.txt_UPDATE}</CmmsText>
                </TouchableOpacity>
                </View>
                </BottomSheet>
                {showTimePicker && (
        <DateTimePicker
          ...
          value={new Date(isFromTime?selectedActivity?selectedActivity.FromDate:0:selectedActivity?selectedActivity.ToDate:0)}
          mode={mode}
          // is24Hour={true}
