...
export function statusDetails(status) {
	switch (status) {
		...
		case 4: return {status: 'Cancelled', color: CmmsColors.joPending,}
		default:
			return null;
	}
}
...
export default (JobOrderView = (props) => {
	const { item, IsAssignChecked} = props;
	// const{item} = props
	...
	return (
		<View
			style={{
				backgroundColor: statusDetails(item.AllotmentStatus).color,
				borderRadius: 10,
				// minHeight: 70,
				// marginStart: 25,
				// paddingStart: 20,
		...
		>
			...
			>
				...
				>
					<Icon name="clock-o" size={12} color="white" /> {Math.floor(item.EHours / 60)}h
				</CmmsText>
				<CmmsText
                  style={{
                    fontSize:12,
			...
			<Text
				...
				style={{
					...
					color: 'white',
					marginBottom: 4,marginEnd:props.isSelectionMode?15:5,
				}}
			>
			...
			{/* <View
style={{flexDirection:'row',bottom:2,
...
>{statusDetails(item.Status).status}</Text>}
		</View>
	);
});
