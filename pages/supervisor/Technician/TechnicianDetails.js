...
export default TechnicianDetails = ({ navigation, route: { params } }) => {
    ...
    useEffect(() => {
        ...
        requestWithEndUrl(`${API_TECHNICIAN}GetWorkStatusPieGraphByDate`, { CurrentDate: dateTime, Date: selectedTimemillies, SEID: params.SEID })
            .then(res => {
                if (res.status != 200) {
        ...
        requestWithEndUrl(`${API_TECHNICIAN}GetJOScheduleByDate`, { CurrentDate: dateTime, Date: selectedTimemillies, SEID: params.SEID })
            .then(res => {
                if (res.status != 200) {
                    throw Error(res.statusText);
                }
                    ...
                    return (
                        <GestureRecognizer
                            ...
                            onSwipeLeft={(state) => {
                                ...
                                if(state.dx < -70)navigation.navigate("TodayJobOrderIssued", { id: item.JOID,SEID: params.SEID })
                            }}
                            onSwipeRight={(state) => {
                                if(state.dx > 70){
                                console.log("jobid: ", item)
                                // if (item.From != 0 && item.To != 0) {
                                requestWithEndUrl(`${API_TECHNICIAN}GetServiceReportDetailsByJOID`, { SEID: params.SEID, ID: item.JOID })
                                    .then((response) => {
                                        // SetJobOrderReport(response.data)
                                        console.log('jobOrderReport Tech det ', response.data)
                                        console.log('jobOrderReport JOID Tech det ', response.data.JOID)
