...
export default (Notes = ({ navigation, route: { params } }) => {
  ...
  const [selectedImg,setSelectedImg] = useState('')
	...
	function getImageList() {
		...
		requestWithEndUrl(`${API_TECHNICIAN}GetImageOfJOBySE`, { JOID: params.JOID, SEID: TechnicianID })
			.then((res) => {
				...
				RNGRP.getRealPathFromURI(imageURI.assets[0].uri).then((path) => {
					...
					PhotoEditor.Edit({
						...
						onDone: (imagePath) => {
							...
							// 	type: 'image/*'
							// });
							bodyFormData.append('JODetails', JSON.stringify({ JOID: params.JOID, SEID: TechnicianID }));
							let imgUri = Platform.OS === 'android' ? `file://${imagePath}` : imagePath;
							var photo = {
	...
	useEffect(() => {
		...
		//{"JOID":4134,"SEID":52}
		dispatch(actionSetLoading(true));
		requestWithEndUrl(`${API_TECHNICIAN}GetNoteInJobTime`, { JOID: params.JOID, SEID: TechnicianID })
			.then((res) => {
     ...
     */}
				...
				<TouchableOpacity
					...
					onPress={() => {
						if (newNote !== '') {
							...
							requestWithEndUrl(
								...
								.then((data) => {
									...
									dispatch(actionSetLoading(false));
									if (data.isSucess) {
										setExistingNotes((existingNotes) => [ ...existingNotes, { Notes: newNote } ]);
										setIsAdd(true);
										setNewNote('');
									}
									// alert(data.Message)
						...
						} else alert(AppTextData.txt_Please_enter_your_notes);
					}}
				>
