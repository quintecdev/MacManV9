...
const useTimer = (initialState = 0, techId = 0, navigation, panelRef) => {
  ...
  const handleStart = (JOID, assignedTechList = [], onCancel = () => {}) => {
    ...
    if (!isActive) {
      ...
      requestWithEndUrl(
        `${API_TECHNICIAN}StartJO`,
        {TechnicianID: [...new Set([techId, ...assignedTechList])], JOID},
        'POST',
      )
        .then((res) => {
        ...
        .then((data) => {
          console.log('StartJO Call api data--->>>', data);
          dispatch(actionSetLoading(false));
          if (data.isSucess) {
            console.log('StartJo api success');
            onCancel();
            panelRef.current.togglePanel();
            handleLocalStart();
            dispatch(actionSetRefreshing(true));
          } else {
            console.log('StartJo api failed ');
            alert(data.Message + 'Timer Start error');
          }
        })
        .catch((err) => {
          dispatch(actionSetLoading(false));
  ...
  const handlePause = (selectedReasonId, JOID) => {
    ...
    if (!isPaused) {
      ...
      requestWithEndUrl(
        `${API_TECHNICIAN}BreakJO`,
        {TechnicianID: [techId], ReasonID: selectedReasonId, JOID},
        'POST',
      )
        .then((res) => {
        ...
        .then((data) => {
          ...
          if (data.isSucess) {
            ...
            alert(data.Message + 'timer Pause Error');
          }
        })
        .catch((err) => {
          dispatch(actionSetLoading(false));
  ...
  const handleResume = (JOID) => {
    ...
    if (isPaused && isActive) {
      ...
      console.log('Params:', techId, JOID);
      requestWithEndUrl(
        `${API_TECHNICIAN}ContinueJO`,
        {TechnicianID: [techId], JOID},
        'POST',
      )
        .then((res) => {
        ...
        .then((data) => {
          ...
          if (data.isSucess) {
            ...
            console.log('error in the handle resume->');
            alert(data.Message);
          }
        })
        .catch((err) => {
  ...
  const handleReset = (JOID, ServiceType) => {
    ...
    console.log('Params:', techId, JOID);
    requestWithEndUrl(
      `${API_TECHNICIAN}StopJO`,
      {TechnicianID: [techId], JOID},
      'POST',
    )
      .then((res) => {
        console.log('URL_Stopjo', {res});
        if (res.status != 200) {
          throw Error(res.statusText);
        }
        return res.data;
      ...
      .then((data) => {
        console.log('data from handleReset---->>', data);
        if (data.isSucess) {
          clearTimer();
          // panelRef.current.togglePanel();
           //it will make the drawer opened after closing the job
          ...
          console.log('is success false result---->>', data.message);
          alert(data.Message);
        }
      })
      .catch((err) => {
  ...
  // newly added by vbn
  const StartTimer = (onCancel = () => {}) => {
    console.log('-----Start Timer Function before if condition-----');
    // if (!isActive) {
  ...
  const PauseTimer = () => {
    console.log('-----Pause Timer Function-----');
    if (!isPaused) {
      console.log('-----inside Pause Timer Function-----');
      BackgroundTimer.stopBackgroundTimer();
      setIsPaused(true);
      // dispatch(actionSetRefreshing(true));
    }
  ...
  const ResumeTimer = () => {
    ...
    if (isPaused && isActive) {
      console.log('-----inside Resume Timer Function-----');
      setIsPaused(false);
      // handleLocalStart()
      BackgroundTimer.runBackgroundTimer(() => {
        setTimer((timer) => timer + 1);
