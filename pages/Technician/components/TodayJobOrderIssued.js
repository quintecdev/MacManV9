...
import { SafeAreaView, View, Text, ImageBackground, FlatList, StyleSheet, ScrollView } from 'react-native';
import axios from 'axios';
import { compareAsc, format } from 'date-fns';
import requestWithEndUrl from '../../../network/request';
import { useSelector, useDispatch } from 'react-redux';
import Icon from 'react-native-vector-icons/FontAwesome';
import CmmsColors, { joGreenAlpha, joSilverAlpha, joYellowAlpha } from '../../../common/CmmsColors';
...
export default TodayJobOrderIssued = ({ navigation, route: { params } }) => {
    ...
    useEffect(() => {
		...
		requestWithEndUrl(`${API_TECHNICIAN}getchecklistassetlist`,{ JOID: params.id, SEID: params.SEID })
			.then((res) => {
	...
	useEffect(() => {
        dispatch(actionSetLoading(true))
		requestWithEndUrl(`${API_TECHNICIAN}GetJobOrderDetailsByID`, { ID: params.id, SEID: params.SEID })
			.then((res) => {
                dispatch(actionSetLoading(false))
				if (res.status != 200) {
		...
		<SafeAreaView style={{ flex: 1 }}>
			...
			{Object.keys(jobOrderIssued).length > 0 && (
				<View style={{flex:1, padding: 8, paddingHorizontal: 8,}}>
					...
					{jobOrderIssued.SEDtls.map((item,index)=><View
								style={{
									flexDirection: 'row',
									alignItems: 'center',
									marginTop: 5
							...
							>
								...
								<CmmsText style={[ styles.SubHeadBottomList, { marginStart: 5 } ]}>
									{item.SE} - {item.TypeOfActivity} -{' '}
									{item.Date != 0 ? format(new Date(item.Date), 'dd/MM/yyyy hh:mm a') : ''}
									{item.ToDate != 0 ? (
										` to ${format(new Date(item.ToDate), 'dd/MM/yyyy hh:mm a')}`
									) : (
										''
									)}
								</CmmsText>
			...
			buttonSize={14}
					...
					showsHorizontalScrollIndicator={false} >
						...
						{/* </View> */}
						{jobOrderIssued.ActivityDtls.length>0&&<View >
						<CmmsText style={styles.SubheadDet}>{AppTextData.txt_Activity_Details}</CmmsText>
								{jobOrderIssued.ActivityDtls.map((item,index)=><CmmsText style={styles.item}>
										{index + 1}. {item.Activity} ({item.TypeOfActivity}-{convertMinsToTime(item.MHrs)}Hrs)
									</CmmsText>)}
							{/* <FlatList
						...
						{jobOrderIssued.SparePartsDtls.length>0&&<View>
						<CmmsText style={styles.SubheadDet}>{AppTextData.txt_Spare_Parts_Required}</CmmsText>
							{jobOrderIssued.SparePartsDtls.map((item,index)=><CmmsText style={styles.item}>
                        ...
                        {jobOrderIssued.Remarks!=''&&<View >
						<CmmsText style={styles.SubheadDet}>{AppTextData.txt_Remark}</CmmsText>

						<CmmsText style={styles.item}>{jobOrderIssued.Remarks}</CmmsText>
                        </View>}
						{params.ServiceType==2&&<><CmmsText style={styles.SubheadDet}>{'Assets'}</CmmsText>
						<DynamicSearchBar 
