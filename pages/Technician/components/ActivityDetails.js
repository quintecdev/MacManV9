...
import { CmmsText, CmmsTextStyles, CTextTitle } from '../../../common/components/CmmsText';
...
export default ActivityDetails = ({ navigation, route: { params } }) => {
	...
	const [additionalActivityDtls,setAdditionalActivityDtls] = useState(JobOrderReportData.AdditionalActivityDtls||[])
    const [newNote,setNewNote] = useState(JobOrderReportData.AdditionalActivityNote)
		...
		<SafeAreaView style={{ flex: 1 }}>
			...
			<View style={{ flex: 1, padding: 8 }}>
				{/* <Text style={styles.MainHead}>{AppTextData.txt_Additional_Activity_Details}</Text> */}
						...
						<Picker
							style={[CmmsTextStyles.textTitle,{color:'',fontFamily: 'sans-serif-condensed',fontSize:16}]}
							...
							onValueChange={(item, index) => {
								if(index!=0&&
									additionalActivityDtls.filter(
										addAct=>addAct.ActivityID==item.ActivityID).length==0){
											setAdditionalActivityDtls(additionalActivityDtls=>[...additionalActivityDtls,{...item,Status:0}])
									
								}
							
							}}
							>
							{[{Activity:activityDetails.length>0?'Select to add activity':'Activity not found',ActivityID:-1},...activityDetails].map((item, index) => {
								return (
									<Picker.Item
									color={index==0?CmmsColors.logoBottomGreen:''}
									key={index} label={`${item.Activity}`} 
									value={item} />
								);
							})}
						</Picker>
				...
				<CTextTitle
				style={{backgroundColor:CmmsColors.logoTopGreen,
					paddingVertical:8,paddingHorizontal:7,marginTop:10}}
				>{AppTextData.txt_Additional_Activities}</CTextTitle>			
				<FlatList
						...
						return (
							<GestureRecognizer
								...
								onSwipeLeft={(state) => {							
									...
									tempAdditionalActDtls[index] = {...item,Status:item.Status == 2?0:2}
									setAdditionalActivityDtls(tempAdditionalActDtls)
									// dispatch(actionSetJobOrderReport({...JobOrderReportData}))
								}}
								onSwipeRight={(state) => {
									let tempAdditionalActDtls = [...additionalActivityDtls]
									tempAdditionalActDtls[index] = {...item,Status:item.Status == 1?2:1}
									setAdditionalActivityDtls(tempAdditionalActDtls)
								...
								style={{
									...
									<View
										style={[{marginBottom:8,paddingHorizontal:4},item.Status == 0 ? styles.ActivityPending : item.Status == 2 ? styles.ActivityWIP : styles.ActivityClosed]}
										>
										<CmmsText
											numberOfLines={2}
											style={{
				...
				<TouchableOpacity
					...
					onPress={() => {
						if(additionalActivityDtls.length>0){
							if (params.isCheckList) params.updateActList(additionalActivityDtls);
							else {
						dispatch(
				...
				>
					<CmmsText style={{ color: 'white', fontWeight: '900' }}>{AppTextData.txt_OK}</CmmsText>
				</TouchableOpacity>
			</View>
		</SafeAreaView>
	);
	...
	function fetchActivityDetails(id) {
		requestWithEndUrl(`${API_TECHNICIAN}GetAdditionalActivity`, { SEID: params.SeId, JOID: params.JoId, ACATID: id })
			.then((res) => {
				setActivityDetails([])
