...
const xmlLoginBg = `<svg width="420pt" height="780pt" viewBox="0 0 420 780" version="1.1" xmlns="http://www.w3.org/2000/svg">
<g id="#ffffffff">
...
export default LoginScreen = ({ navigation,route:{params} }) => {
    // global.roundedScreenWidth = Math.round(screenWidth)
...
<KeyboardAvoidingView
           ...
           <SvgXml 
            ...
            xml={
              ...
              /> */}
                ...
                >
                    <Icon name="user" size={18} color={CmmsColors.colorWithAlpha('white',0.5)} />
                    <TextInput
                ...
                <View
                    style={{...styles.textInputContainer,marginTop:20}}
                >
                    <Icon name="key" size={18} color={CmmsColors.colorWithAlpha('white',0.5)} />
                    <TextInput
                        ref = {pwdIpRef}
                        style={styles.textInput}
                ...
                >
                    <CTextHint
                    >{AppTextData.txtForgot_Password}</CTextHint>
                </TouchableOpacity>
                ...
                >
                    <Text style={{ color: CmmsColors.colorWithAlpha('white',0.8), fontSize: 16, fontWeight: 'bold' }}>{AppTextData.txtLogin}</Text>
                </TouchableOpacity>
                ...
                >
                    <Text style={{color:CmmsColors.colorWithAlpha('white',0.5),fontSize:12,fontWeight:'900'}}>{selectedLng?.Language}</Text>
                </TouchableOpacity>
    ...
    renderItem={({item})=><TouchableOpacity>
        <Icon name={selectedLng.Languagevalue!=item.Languagevalue? "check-square-o":"square-o"} size={18} color="black" />
        <CText style={{color:'black'}}>{item.Language}</CText>
        </TouchableOpacity>
        }
    />
    </Dialog> */}
    ...
    function doLogin(token="") {    
        ...
        requestWithEndUrl(`${API_TECHNICIAN}Login`, {...User,"Token":token}, 'POST')
            .then(res => {
            ...
            .then(data => {
                ...
                if (data.isSucess) {
                    ...
                    const loggedUser = JSON.stringify({ TechnicianID, TechnicianName,UserType })
                    console.log("login_success: ", { loggedUser })
                    dispatch(actionSetLoginData({ TechnicianID, TechnicianName,UserType }))
